# Email / Newsletter 环境分组需求

## 1. 目标

本分组定义 `Email / Newsletter` 相关环境变量的采集、确认、回写与最终落地规则。
...
```bash
mksaas env email-newsletter [--profile test|prod]
```
...
```mermaid
flowchart TD
    A[执行 mksaas env email-newsletter] --> B[读取 setup-state.json]
    B --> C{是否已有 Email / Newsletter 配置}
    C -->|是| D[展示已有值与变量用途]
    C -->|否| E[进入 Email / Newsletter 采集流程]
    D --> F{是否修改}
    F -->|否| G[沿用已有配置]
    F -->|是| E
    E --> H[逐项采集并校验输入]
...
sequenceDiagram
    ...
    U->>C: mksaas env email-newsletter
    C->>J: 读取 email-newsletter 分组配置
    J-->>C: 返回已有值或空结果
    C->>U: 展示已有值、变量用途与缺失项
    U->>C: 选择沿用或修改
    alt 需要补充配置
    ...
    C->>C: 校验字段格式与必填项
    C->>J: 回写 email-newsletter 分组配置
    C-->>U: 提示需在 apply 阶段统一落地并可用 pnpm run dev 验证
```

## 8. 采集要求

1. 支持 Resend、Beehiiv 或组合使用
2. 若已有值，先展示已配置状态并询问是否修改
3. 提示用户先到对应平台生成 API Key 或 Publication ID

## 9. 生成要求

1. API Key 写入 `secrets.*.env`
2. `BEEHIIV_PUBLICATION_ID` 可写入 `.env.*`
3. 未启用 provider 可跳过输出

## 10. 安全要求

1. `RESEND_API_KEY` 与 `BEEHIIV_API_KEY` 必须标记为敏感字段
2. 不得在终端明文展示
