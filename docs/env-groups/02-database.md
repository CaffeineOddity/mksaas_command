...
本分组定义 `Database` 相关环境变量的采集、确认、回写与最终落地规则。
...
```bash
mksaas env database [--profile test|prod]
```
...
```mermaid
flowchart TD
    A[执行 mksaas env database] --> B[读取 setup-state.json]
    B --> C{是否已有 Database 配置}
    C -->|是| D[展示已有值与变量用途]
    C -->|否| E[进入 Database 采集流程]
    D --> F{是否修改}
    F -->|否| G[沿用已有配置]
    F -->|是| E
    E --> H[逐项采集并校验输入]
...
sequenceDiagram
    ...
    U->>C: mksaas env database
    C->>J: 读取 database 分组配置
    J-->>C: 返回已有值或空结果
    C->>U: 展示已有值、变量用途与缺失项
    U->>C: 选择沿用或修改
    alt 需要补充配置
    ...
    C->>C: 校验字段格式与必填项
    C->>J: 回写 database 分组配置
    C-->>U: 提示需在 apply 阶段统一落地并可用 pnpm run dev 验证
```

## 8. 采集要求

1. 支持 Neon、Supabase、Local Postgres 等 provider
2. 若已有值，先摘要展示，不回显完整连接串
3. 允许按 `test` 与 `prod` 分别配置，并提示用户先完成数据库平台配置

## 9. 生成要求

1. 写入 `secrets.test.env` 与 `secrets.prod.env`
2. 缺少必填连接串时不得执行最终 apply
3. provider 选择写入 JSON 模块配置

## 10. 安全要求

1. `DATABASE_URL` 必须标记为敏感字段
2. 不得在终端打印完整连接串
