...
本分组定义 `Cron Jobs` 相关环境变量的采集、确认、回写与最终落地规则。
...
```bash
mksaas env cron-jobs [--profile test|prod]
```
...
1. `CRON_JOBS_USERNAME`
2. `CRON_JOBS_PASSWORD`
...
```mermaid
flowchart TD
    A[执行 mksaas env cron-jobs] --> B[读取 setup-state.json]
    B --> C{是否已有 Cron Jobs 配置}
    C -->|是| D[展示已有值与变量用途]
    C -->|否| E[进入 Cron Jobs 采集流程]
    D --> F{是否修改}
    F -->|否| G[沿用已有配置]
    F -->|是| E
    E --> H[逐项采集并校验输入]
...
sequenceDiagram
    ...
    U->>C: mksaas env cron-jobs
    C->>J: 读取 cron-jobs 分组配置
    J-->>C: 返回已有值或空结果
    C->>U: 展示已有值、变量用途与缺失项
    U->>C: 选择沿用或修改
    alt 需要补充配置
...
1. 若已有值，仅展示已配置状态与摘要
2. 支持按 profile 分别维护
3. 提示用户明确 Basic Auth 用户名密码的用途后再填写
