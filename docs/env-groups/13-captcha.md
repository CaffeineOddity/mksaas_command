...
本分组定义 `Captcha` 相关环境变量的采集、确认、回写与最终落地规则。
...
```bash
mksaas env captcha [--profile test|prod]
```
...
```mermaid
flowchart TD
    A[执行 mksaas env captcha] --> B[读取 setup-state.json]
    B --> C{是否已有 Captcha 配置}
    C -->|是| D[展示已有值与变量用途]
    C -->|否| E[进入 Captcha 采集流程]
    D --> F{是否修改}
    F -->|否| G[沿用已有配置]
    F -->|是| E
    E --> H[逐项采集并校验输入]
...
1. 支持 Cloudflare Turnstile
2. 若已有配置，先展示 site key 摘要与 secret 已配置状态
3. 提示用户先在 Cloudflare 创建站点并获取 key

## 9. 生成要求

1. `NEXT_PUBLIC_TURNSTILE_SITE_KEY` 写入 `.env.*`
2. `TURNSTILE_SECRET_KEY` 写入 `secrets.*.env`
3. 未启用时可跳过输出

## 10. 安全要求

1. `TURNSTILE_SECRET_KEY` 必须标记为敏感字段
2. 不得打印完整 secret
