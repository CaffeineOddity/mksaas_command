...
## Feature F10：build.config.json + build.sh

**先读：** docs/build_install_upgrade_uninstall.md §2/§3/§4；REQUIREMENTS §12

**Files:** `build.config.json`、`mksaas/version.py`、`mksaas/paths.py`、`build.sh`、`tests/test_version.py`

**测试用例：**
- `read_version()` 解析；`version_string` debug=`-devN`/release 无后缀
- `bump` patch/minor/major + build 重置 1
- build.sh 默认 debug 路径 `.build/dist/<v>-dev<next_build>/mksaas`；成功后将 build 回写为本次产物号；--release 不变；--bump 重置 build=0；--minor 单用报错
...
- install.sh 来源判定：默认装源码入口；仅在 `--version` 时安装 `.build/dist` 指定产物
