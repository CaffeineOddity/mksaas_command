...
本步骤负责建立 `.mksaas/setup-state.json`，并完成仓库与项目级基础信息的采集。它是单步命令，可单独运行、可重复运行，也可以被 `mksaas init` 编排调用。

说明：

1. `project` 负责初始化状态文件与仓库配置
2. `project` 不直接执行 clone、remote 绑定或 push
3. 具体环境变量采集通过 `mksaas env <group> [--profile test|prod]` 完成
...
4. 初始化 `steps.project` 与 `steps.apply` 状态
...
sequenceDiagram
    ...
    C->>J: 回写 project、steps.project 与默认顶层结构
