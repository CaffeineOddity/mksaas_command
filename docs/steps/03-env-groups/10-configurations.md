# Configurations 环境分组需求

## 1. 目标

本分组定义 `Configurations` 相关环境变量的采集、确认、回写与最终落地规则。
...
```bash
mksaas env configurations [--profile test|prod]
```
...
1. `DISABLE_IMAGE_OPTIMIZATION`
2. `NEXT_PUBLIC_DEMO_WEBSITE`
...
```mermaid
flowchart TD
    A[执行 mksaas env configurations] --> B[读取 setup-state.json]
    B --> C{是否已有 Configurations 配置}
    C -->|是| D[展示已有值与变量用途]
    C -->|否| E[进入 Configurations 采集流程]
    D --> F{是否修改}
    F -->|否| G[沿用已有配置]
    F -->|是| E
    E --> H[逐项采集并校验输入]
...
sequenceDiagram
    ...
    U->>C: mksaas env configurations
    C->>J: 读取 configurations 分组配置
    J-->>C: 返回已有值或空结果
    C->>U: 展示已有值、变量用途与缺失项
    U->>C: 选择沿用或修改
    alt 需要补充配置
