...
本分组定义 `AI` 相关环境变量的采集、确认、回写与最终落地规则。
...
```bash
mksaas env ai [--profile test|prod]
```
...
6. `GOOGLE_GENERATIVE_AI_API_KEY`
7. `DEEPSEEK_API_KEY`
...
3. 先让用户选择一种 AI provider，例如 OpenAI、OpenRouter、DeepSeek、Google Generative AI 等
4. 根据所选 provider，只展示并采集对应的 API Key 或 Token 字段，而不是把所有 AI provider 字段一起逐项询问
5. 告知用户该 provider 字段的用途，并提示先到对应平台创建 API Key 或 Token
6. 对输入值做基础校验，例如密钥或 token 是否为空
...
```mermaid
flowchart TD
    A[执行 mksaas env ai] --> B[读取 setup-state.json]
    B --> C{是否已有 AI 配置}
...
1. 进入本分组后必须先选择一种 AI provider
2. 仅采集当前所选 provider 对应字段，不应把所有 provider 的字段一起展示
3. 若已有值，仅展示当前已选择的 provider 与摘要
4. 提示用户先到对应 AI 平台生成 API Key 或 Token
