...
本分组定义 `Crisp` 相关环境变量的采集、确认、回写与最终落地规则。
...
```bash
mksaas env crisp [--profile test|prod]
```
...
1. `NEXT_PUBLIC_CRISP_WEBSITE_ID`
...
```mermaid
flowchart TD
    A[执行 mksaas env crisp] --> B[读取 setup-state.json]
    B --> C{是否已有 Crisp 配置}
    C -->|是| D[展示已有值与变量用途]
    C -->|否| E[进入 Crisp 采集流程]
    D --> F{是否修改}
...
1. 若已有值，先展示并确认是否修改
2. 支持按 profile 分别维护
3. 提示用户先在 Crisp 创建网站并获取 Website ID
