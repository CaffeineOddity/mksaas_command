...
仓库根目录维护一个版本状态文件（如 `VERSION`），包含两个字段：
...
6. `--minor` 与 `--major` 仅在与 `--bump` 组合时生效，单独使用应报错提示
...
7. 构建成功后打印产物路径（含版本字符串）与文件大小
8. debug 构建完成后必须将 `build` 字段 `+1` 回写状态文件；release 构建不递增 `build`
...
2. 安装方式为「本地目录 + 符号链接」：将 `mksaas` 可执行文件放入安装目录，并在 PATH 可达目录建立 `mksaas` 符号链接指向它
...
2. 从构建产物目录读取 PyInstaller 单文件二进制，替换安装目录中的旧版可执行文件
3. 升级前校验产物是否存在；不存在时提示用户先执行 `build.sh`
4. 升级前展示当前已安装版本与产物版本（产物内嵌的版本字符串，含 `-devN` 或 release 形态），让用户确认
5. 升级保留符号链接不动（指向同一安装路径），仅替换被指向的可执行文件
6. 升级完成后打印新版本信息
7. 升级失败时不得破坏旧版可执行文件（建议先写入临时文件再原子替换）
8. `upgrade --local` 默认从构建产物目录中最新版本子目录（按 §3 版本字符串排序取最大）读取；也支持显式指定版本子目录
